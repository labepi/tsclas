...
for(tau in tau_l)
{
    ...
    csi = class_separability(ds$H, ds$C, ds$k, D=D)
...
#ggplot(ds, aes(x=H, y=C, color=k)) + geom_point()

quit()


#########################
## fig. separability index
#########################

csi.df = data.frame(x=tau_l, y=csi_l)

p = ggplot(csi.df, aes(x=x, y=y)) + 
    geom_vline(xintercept=2, color=2, size=2, alpha=0.8) +
    geom_line() + geom_point(size=3) +
    theme_bw() +
    xlab(expression(tau)) +
    ylab(expression(S[I])) +
    annotate(geom="text", x=6.2, y=0.79, 
             label=expression(paste(S[I],'=0.798')), 
             color="red", size=12) +
    annotate(geom="text", x=4.2, y=0.46, 
             label=expression(paste(tau,'=2')), 
             color="red", size=14) +
    theme(text=element_text(size=36)) 

ggsave('img/fig_separability_test.pdf', p, width=10, height=6)
...
#element_text(size=16, family='Sans'))
...
ggsave('img/fig_arrange_besttau.pdf', p, width=14, height=10)
